...
color_df = read.csv("/data/wangxin/ref/discrete_color.csv", header = T)
...
expansion_ls = c("0.25", "0.9", "0.95", "1.0")
...
main_dir = paste0("/syn1/wangxin/work/mtsim/0421/mt_freq/",dif_mode,"/")
out_dir = "/syn1/wangxin/work/mtsim/0421/"
simul_id = args_2
gen = args_3
expansion = expansion_ls[exp_idx] 

mat_name = paste0("mt_mut_freq_", bn, "_0.1_", simul_id, "_", gen, "_0.4_",expansion,"_rs.mtx")
cell_name = paste0("mt_mut_freq_", bn, "_0.1_", simul_id, "_", gen, "_0.4_",expansion,"_rs_cells.csv")
mut_name = paste0("mt_mut_freq_", bn, "_0.1_", simul_id, "_", gen, "_0.4_",expansion,"_rs_muts.csv")
...
conditions_subset.tib <- conditions.tib %>% filter(min_clone_size %in% c(10,20,50), min_vaf %in% c("n10","n20","n50"))
...
CIV_file_name = paste0(out_dir,"CIV/CIV_clones_", simul_id, "_", gen, "gen_", expansion, ".csv")
...
pre_df = read.csv("/syn1/wangxin/work/mtsim/0421/pre_id.csv", head=T, row.names=1)
mut_state = ifelse(as.numeric(rownames(new_plot_mat)) < pre_df[as.character(simul_id),], "pre-existing", "de novo")
CIV_type_df = data.frame(CIV = CIV, Type = mut_state, Simulation = rep(simul_id, length(CIV)), Model = expansion)
CIV_type_file_name = paste0(out_dir, "CIV/mut_type_", simul_id, "_", gen, "gen_", expansion, ".csv")
...
heatmap_name = paste0(out_dir,"plots/", dif_mode, "_", simul_id, "_",gen,"_gen_", expansion, "_CIV_heatmap.pdf")
...
if (expansion == "1.0") {
  ...
  tree_name = paste0("clonal_expansion_tree_", simul_id, "_", expansion, "_", gen, ".nwk")
  tree = read.newick(paste0(out_dir,"trees/original_trees/",tree_name))
  clone_df = data.frame(Clone=clone_info_df$Clone, row.names = clone_info_df$Cell)
  to_drop = setdiff(tree$tip.label, rownames(clone_df))
  new_tree = drop.tip(tree, to_drop)}
...
tree_pdf_name = paste0(out_dir,"plots/celldivision_tree_", dif_mode, "_", simul_id, "_",gen,"_gen_", expansion, ".pdf")
ggsave(tree_pdf_name, p2, height = 2.3, width = 5)
message(paste0("CIV heatmaps have been saved to: ", tree_pdf_name))
...
for (x in 1:length(CIV)) {
        #all positive cells
        ori_cells = colnames(new_plot_mat1)[new_plot_mat1[CIV[x],] >= 0.05]
        ...
        ori_cells = ori_cells[tmp_vaf_ls >= 0.05]
...
original_tree_file = paste0(out_dir,"trees/original_trees/",tree_name)
CIV_pos_cell_df = as.data.frame(matrix(ncol=2, nrow=0))
colnames(CIV_pos_cell_df) = c("Cell", "Clone")
for (i in 1:length(CIV)) {
    tmp_df = data.frame("Cell" = colnames(new_plot_mat1)[new_plot_mat1[i,] > 0.05], "Clone" = paste0("C",i))
    CIV_pos_cell_df =  rbind(CIV_pos_cell_df, tmp_df)                   
}
CIV_pos_cell_file = paste0(out_dir,"CIV/CIV_postive_cells_", simul_id, "_", gen, "gen_", expansion, ".csv")
write.csv(CIV_pos_cell_df, file = CIV_pos_cell_file, quote = F, row.names = F)
message("Clone info has been saved to: ", CIV_pos_cell_file)

all_cc_output_file = paste0(out_dir, "CIV/CIV_composition_", simul_id, "_", gen, "gen_", expansion, "_all.csv")
all_anc_output_file = paste0(out_dir, "CIV/anc_composition_", simul_id, "_", gen, "gen_", expansion, "_all.csv")
...
cc_output_file = paste0(out_dir, "CIV/CIV_composition_", simul_id, "_", gen, "gen_", expansion, "_sub.csv")
anc_output_file = paste0(out_dir, "CIV/anc_composition_", simul_id, "_", gen, "gen_", expansion, "_sub.csv")
...
meanvar_df_all$"AG" = all_tmp_df$Ancestor_gen
meanvar_df_all$"AC" = all_tmp_df$Ancestor
meanvar_df_sub$"AG" = sub_tmp_df$Ancestor_gen
meanvar_df_sub$"AC" = sub_tmp_df$Ancestor


meanvar_df = rbind(meanvar_df_all, meanvar_df_sub)
meanvar_df$Model = expansion
meanvar_file_name = paste0(out_dir,"CIV/meanvar_CIV_", simul_id, "_", gen, "gen_", expansion, ".csv")
...
p3 = ggplot(compo_df, aes(x = clone, y=counts, fill = ancestor)) + geom_bar(stat="identity") + theme_classic() + labs(title = paste0(simul_id, " - ", gen, expansion)) + 
...
p4 = ggplot(compo_df, aes(x = clone, y=Proportion, fill = ancestor)) + geom_bar(stat="identity") + theme_classic() + labs(title = paste0(simul_id, " - ", gen, expansion)) + 
...
cc_pdf_file = paste0(out_dir, "plots/lineage_composition_", simul_id, "_", gen, "gen_",expansion,".pdf")
ggsave(cc_pdf_file, p4, height = 2.2, width = 2.2)
message(paste0("PDF has been written to: ",cc_pdf_file))

meta_file_name = paste0("/syn1/wangxin/work/mtsim/0421/res_0421/", dif_mode, "/", simul_id, "/tree_origin_", dif, "_", simul_id, ".csv")
...
p5 = ggplot(cell_type_df, aes(x = Clone, y= Counts, fill = Type)) + geom_bar(stat="identity") + theme_classic() + labs(title = paste0(simul_id, " - ", gen, expansion)) + 
        theme(plot.title=element_text(hjust=0.5, size=9), axis.text = element_text(size = 9), axis.title = element_text(size=9),
...
p6 = ggplot(cell_type_df, aes(x = Clone, y= Proportion, fill = Type)) + geom_bar(stat="identity") + theme_classic() + labs(title = paste0(simul_id, " - ", gen, expansion)) + 
        theme(plot.title=element_text(hjust=0.5, size=9), axis.text = element_text(size = 9), axis.title = element_text(size=9),
...
ct_pdf_file = paste0(out_dir, "plots/celltype_composition_", simul_id, "_", gen, "gen_",expansion,".pdf")
...
if (expansion != "1.0") {
  tsne_file = paste0("/syn1/wangxin/work/mtsim/0421/tsne/tsne_",simul_id,"_", gen, "gen_",expansion, "_rs_dis10.csv")
  ...
  tsne_pdf_file = paste0(out_dir, "/plots/eachCIV_on_tsne_",simul_id, "_", gen, "gen_", expansion, "_dis10.pdf")
  ggexport(plotlist = plot_ls, nrow=4, ncol=4, filename = tsne_pdf_file, width = 20, height = 20)
  
  tsne_file = paste0("/syn1/wangxin/work/mtsim/0421/tsne/tsne_",simul_id,"_", gen, "gen_",expansion, "_rs_dis12.csv")
  tsne_df = read.csv(tsne_file, check.names = F, row.names =1)
  colnames(tsne_df) = c("tSNE1", "tSNE2")
  plot_ls = lapply(1:CIV_num, function (x) {
      Clone = paste0("C", x)
  ...
  tsne_pdf_file = paste0(out_dir, "/plots/eachCIV_on_tsne_",simul_id, "_", gen, "gen_", expansion, "_dis12.pdf")
  ggexport(plotlist = plot_ls, nrow=4, ncol=4, filename = tsne_pdf_file, width = 20, height = 20)
  
  tsne_file = paste0("/syn1/wangxin/work/mtsim/0421/tsne/tsne_",simul_id,"_", gen, "gen_",expansion, "_rs_dis14.csv")
  tsne_df = read.csv(tsne_file, check.names = F, row.names =1)
  colnames(tsne_df) = c("tSNE1", "tSNE2")
  plot_ls = lapply(1:CIV_num, function (x) {
      Clone = paste0("C", x)
  ...
  tsne_pdf_file = paste0(out_dir, "/plots/eachCIV_on_tsne_",simul_id, "_", gen, "gen_", expansion, "_dis14.pdf")
  ggexport(plotlist = plot_ls, nrow=4, ncol=4, filename = tsne_pdf_file, width = 20, height = 20)
  
} else{
  tsne_file = paste0("/syn1/wangxin/work/mtsim/0421/res_0421/", dif_mode, "/", simul_id, "/tsne_", dif, "_", simul_id, ".csv")
  tsne_df = read.csv(tsne_file, check.names = F, row.names =1)
  colnames(tsne_df) = c("tSNE1", "tSNE2")
  plot_ls = lapply(1:CIV_num, function (x) {
      Clone = paste0("C", x)
  ...
  tsne_pdf_file = paste0(out_dir, "/plots/eachCIV_on_tsne_",simul_id, "_", gen, "gen_", expansion, ".pdf")
  ggexport(plotlist = plot_ls, nrow=4, ncol=4, filename = tsne_pdf_file, width = 20, height = 20)
